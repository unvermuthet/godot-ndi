...
sys.path.insert(0, root_directory := os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "../../"))

github_url = "https://github.com/unvermuthet/godot-ndi"
            ...
            for constant in constants:
                ...
                if enum is None:
                    ...
                    if enum in class_def.enums:
                        ...
                        enum_def = EnumDef(enum, TypeName(
...
class ScriptLanguageParityCheck:
    ...
    def add_hit(self, class_name: str, context: DefinitionBase, error: str, state: State) -> None:
        ...
        if class_def.class_group == "variant" and class_def.name != "Object":
            # Variant types are replaced with native types in C#, we don't expect parity.
            return
...
def main() -> None:
    ...
    parser.add_argument(
        "path", nargs="+", help="A path to an XML file or a directory containing XML files to parse.")
    ...
    group.add_argument("--output", "-o", default=".",
    ...
    if not args.dry_run and args.lang != "en":
        lang_file = os.path.join(
            os.path.dirname(os.path.realpath(__file__)
                            ), "..", "translations", "{}.po".format(args.lang)
        )
        ...
        if os.path.basename(path) in ["modules", "platform"]:
            for subdir, dirs, _ in os.walk(path):
                if "doc_classes" in dirs:
                    ...
                    class_file_names = (f for f in os.listdir(
                        doc_dir) if f.endswith(".xml"))
    ...
    for cur_file in file_list:
        try:
            ...
            print_error(
                f"{cur_file}: Parse error while reading the file: {e}", state)
            continue
        doc = tree.getroot()
...
def make_rst_class(class_def: ClassDef, state: State, dry_run: bool, output_dir: str) -> None:
    ...
    with open(
        os.devnull if dry_run else os.path.join(
        ...
        source_xml_path = os.path.relpath(
            class_def.filepath, root_directory).replace("\\", "/")
        ...
        if len(class_def.properties) > 0:
            ...
            for property_def in class_def.properties.values():
                ...
                if default is not None and property_def.overrides:
                    ...
                    ml.append((type_rst, property_def.name,
                              f"{default} (overrides {ref})"))
                else:
        ...
        if len(class_def.constructors) > 0:
            ...
            for method_list in class_def.constructors.values():
                for m in method_list:
                    ml.append(make_method_signature(
...
def make_setter_signature(class_def: ClassDef, property_def: PropertyDef, state: State) -> str:
    ...
    # Otherwise we fake it with the information we have available.
        ...
        setter_params.append(ParameterDef(
            "value", property_def.type_name, None))
        setter = MethodDef(property_def.setter, TypeName(
            "void"), setter_params, None, None)
...
def make_getter_signature(class_def: ClassDef, property_def: PropertyDef, state: State) -> str:
    ...
    # Otherwise we fake it with the information we have available.
        ...
        getter = MethodDef(property_def.getter,
                           property_def.type_name, getter_params, None, None)
...
RESERVED_FORMATTING_TAGS = ["i", "b", "u", "lb",
                            "rb", "code", "kbd", "center", "url", "br"]
...
def format_text_block(
    ...
    while True:
        ...
        while not inside_code:
            ...
            if iter_pos == -1:
                break
            # don't escape within a snake_case word
            if not post_text[iter_pos + 1].isalnum():
...
def sanitize_operator_name(dirty_name: str, state: State) -> str:
    ...
    if clear_name == "!=":
        ...
        clear_name = "xxx"
        print_error(
            f'Unsupported operator type "{dirty_name}", please add the missing rule.', state)
